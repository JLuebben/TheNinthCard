...
public class TouchGesture
{
    ...
    public IEnumerator CheckHorizontalSwipes(Action onLeftSwipe, Action onRightSwipe, Action onFromLeftEdge) //Coroutine, which gets Started in "Start()" and runs over the whole game to check for swipes
    {
        while (true)
                    ...
                    case TouchPhase.Ended:
                        if (isASwipe(touch))
                        {
                            ...
                            if (Mathf.Sign(touch.position.x - startPos.x) == 1f) //Swipe-direction, either 1 or -1.
                                ...
                                if (startPos.x < settings.edgeOffset)
                                {
